...
def test_multi_tensor_adam_exceptions():
    ...
    with pytest.raises(AssertionError, match="Expected 4 or 5 tensor lists"):
        multi_tensor_adam_fl(1024, noop_flag, [[torch.randn(2)]], 0.01, 0.9, 0.99, 1e-8, 1, 0, 1, 0.0)
    ...
    with pytest.raises(AssertionError, match="List 1 has 1 tensors, expected 2"):
        tensor_lists = [[torch.randn(2), torch.randn(2)], [torch.randn(2)], [torch.randn(2)], [torch.randn(2)]]
...
def test_param_remainder_noop_shortcircuit():
    ...
    res = multi_tensor_adam_param_remainder_fl(1024, noop_flag, [], 0.01, 0.9, 0.99, 1e-8, 1, 0, 1, 0.0)
    assert res is None
...
def test_param_remainder_bit_manipulation_lifecycle(mode, weight_decay):
    ...
    # Introduce negative remainders to force hit the conditional `torch.where(local_p_rem < 0, ...)` branch
...
def test_param_remainder_invariants():
    ...
    with pytest.raises(AssertionError, match="Expected 5 tensor lists"):
        multi_tensor_adam_param_remainder_fl(1024, noop_flag, [[torch.randn(2)]], 0.01, 0.9, 0.99, 1e-8, 1, 0, 1, 0.0)
...
def test_version_queries_and_stream_constants():
    ...
    with patch("transformer_engine.plugin.core.backends.flagos.flagos.NVTE_Fused_Attn_Backend", create=True) as mock_enum:
...
def test_attention_backend_env_matrix(env_flash, env_fused, env_unfused, expected_flash_idx_0, expect_version_instance):
    ...
    with patch.dict(os.environ, env_mock), \
         patch("transformer_engine.plugin.core.backends.flagos.flagos.NVTE_Fused_Attn_Backend", create=True) as mock_enum:
...
def test_generic_gemm_forward_mapping():
    ...
    with patch(
        ...
        res = backend.generic_gemm(
            A="mat_a", transA=False, B="mat_b", transB=True, D="mat_d", quantizer=None,
            output_dtype=None, bias=None, bias_type=None, gelu=False, gelu_in=None, grad=False,
            workspace="ws", workspace_size=1024, accumulate=False, use_split_accumulator=False
        )
    ...
    with patch(
        ...
        try:
            res = backend.te_general_grouped_gemm(
                A=["a1"], transa=True, B=["b1"], transb=False, D=None, D_type=None, m_splits=[1],
                bias=[], bias_type=None, single_output=True, pre_gelu_out=[], grad=True,
                workspace=[], workspaceSizes=2048, accumulate=True, use_split_accumulator=True, math_sm_count=80
            )
...
def test_multi_tensor_fused_adam_optimizers():
    ...
    with patch(
        ...
        backend.multi_tensor_adam_param_remainder(256, "flag", [["w"]], 0.001, 0.9, 0.99, 1e-8, 1, 0, 1, 0.01)

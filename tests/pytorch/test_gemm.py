...
def test_generic_gemm_lifecycle_matrix(a_ndim, b_ndim, transA, transB, has_bias, grad, has_D, accumulate):
    ...
    res = generic_gemm_fl(
        A=A, transA=transA, B=B, transB=transB, D=D, quantizer=None, output_dtype=4,
        bias=bias, bias_type=None, gelu=False, gelu_in=None, grad=grad,
        workspace=workspace, workspace_size=0, accumulate=accumulate, use_split_accumulator=False
    )
...
def test_generic_gemm_unsupported_features():
    ...
    with pytest.raises(AssertionError, match="do not support gelu now"):
        generic_gemm_fl(
            dummy_tensor, False, dummy_tensor, False, None, None, None,
            None, None, gelu=True, gelu_in=dummy_tensor, grad=False,
            workspace=dummy_tensor, workspace_size=0, accumulate=False, use_split_accumulator=False
        )

    with pytest.raises(AssertionError, match="do not support quantization now"):
        generic_gemm_fl(
            dummy_tensor, False, dummy_tensor, False, None, quantizer="mock", output_dtype=None,
            bias=None, bias_type=None, gelu=False, gelu_in=None, grad=False,
            workspace=dummy_tensor, workspace_size=0, accumulate=False, use_split_accumulator=False
        )
...
def test_grouped_gemm_single_output_validation():
    ...
    with pytest.raises(ValueError, match="D should be allocated for single output case."):
        te_general_grouped_gemm_fl(
            B=[], transb=False, A=[], transa=False, D=None, D_type=None, m_splits=[],
            bias=[], bias_type=None, single_output=True, pre_gelu_out=[], grad=False,
            workspace=[], workspaceSize=0, accumulate=False, use_split_accumulator=False, math_sm_count=0
        )
...
def test_grouped_gemm_standard_lifecycle(grad, accumulate, single_output):
    ...
    returned_bias = te_general_grouped_gemm_fl(
        B=B, transb=False, A=A, transa=False, D=D, D_type=None, m_splits=[4, 4],
        bias=bias, bias_type=None, single_output=single_output, pre_gelu_out=pre_gelu_out, grad=grad,
        workspace=[], workspaceSize=0, accumulate=accumulate, use_split_accumulator=False, math_sm_count=80
    )
...
def test_grouped_gemm_zero_element_inputs(single_output, grad, accumulate):
    ...
    returned_bias = te_general_grouped_gemm_fl(
        B=B, transb=False, A=A, transa=False, D=D, D_type=None, m_splits=[0],
        bias=bias, bias_type=None, single_output=single_output, pre_gelu_out=pre_gelu_out, grad=grad,
        workspace=[], workspaceSize=0, accumulate=accumulate, use_split_accumulator=False, math_sm_count=80
    )
    assert returned_bias == bias

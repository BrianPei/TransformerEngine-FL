...
torch_stensors = [
    ...
    "drelu_torch", "dreglu_torch", "dsrelu_torch", "dsreglu_torch", "dsilu_torch", "dswiglu_torch",
    "clamped_dswiglu_torch", "dbias_dgelu_torch", "dbias_dsilu_torch", "dbias_drelu_torch",
    "dbias_dqgelu_torch", "dbias_dsrelu_torch", "scaled_softmax_forward_torch", "scaled_softmax_backward_torch",
...
def test_multi_tensor_optimizers_and_scales():
    ...
    backend.multi_tensor_adam_capturable(1024, flag, t_list, torch.tensor(1e-3), 0.9, 0.99, 1e-8, torch.tensor(1), 0, 1, 0.01, torch.tensor(1.0))
    backend.multi_tensor_adam_capturable_master(1024, flag, t_list, torch.tensor(1e-3), 0.9, 0.99, 1e-8, torch.tensor(1), 0, 1, 0.01, torch.tensor(1.0))

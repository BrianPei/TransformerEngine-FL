...
mock_flag_gems.sum = lambda x, *args, **kwargs: torch.sum(x) if isinstance(x, torch.Tensor) else torch.tensor(1.0)
mock_flag_gems.mul = lambda x, y, *args, **kwargs: x * y
mock_flag_gems.add = lambda x, y, *args, **kwargs: x + y
mock_flag_gems.sqrt = lambda x, *args, **kwargs: torch.sqrt(x) if isinstance(x, torch.Tensor) else torch.tensor(1.0)
